...
class Closecom {
  ...
  _initLeads() {
    this.lead = {
      search: (options = {}) => {
        ...
        return this._get('/lead/', parameters);
      },
      ...
      convert: (data) => {
        this._validateRequired(data, ['lead_id'], 'Lead Convert');
        return this._post('/lead/convert/', data);
      }
    };
  }
  ...
  _initContacts() {
    this.contact = {
      search: (options = {}) => this._get('/contact/', options),
  ...
  _initActivities() {
    this.activity = {
      search: (options = {}) => this._get('/activity/', options),
      
      note: {
        search: (options = {}) => this._get('/activity/note/', options),
      ...
      email: {
        search: (options = {}) => this._get('/activity/email/', options),
      ...
      call: {
        search: (options = {}) => this._get('/activity/call/', options),
      ...
      sms: {
        search: (options = {}) => this._get('/activity/sms/', options),
      ...
      meeting: {
        search: (options = {}) => this._get('/activity/meeting/', options),
  ...
  _initOpportunities() {
    this.opportunity = {
      search: (options = {}) => this._get('/opportunity/', options),
  ...
  _initTasks() {
    this.task = {
      search: (options = {}) => this._get('/task/', options),
  ...
  _initCustomActivities() {
    this.custom_activity = {
      search: (type, options = {}) => this._get(`/custom_activity/${type}/`, options),
  ...
  _initUsers() {
    this.user = {
      me: () => this._get('/me/'),
      list: (options = {}) => this._get('/user/', options),
  ...
  _initOrganizations() {
    this.organization = {
      list: (options = {}) => this._get('/organization/', options),
  ...
  _initPipelines() {
    this.pipeline = {
      list: (options = {}) => this._get('/pipeline/', options),
  ...
  _initEmailTemplates() {
    this.email_template = {
      search: (options = {}) => this._get('/email_template/', options),
  ...
  _initSavedSearches() {
    this.saved_search = {
      list: (options = {}) => this._get('/saved_search/', options),
  ...
  _initSequences() {
    this.sequence = {
      search: (options = {}) => this._get('/sequence/', options),
  ...
  _initSequenceSubscriptions() {
    this.sequence_subscription = {
      list: (options = {}) => this._get('/sequence_subscription/', options),
  ...
  _initReports() {
    this.report = {
      list: (options = {}) => this._get('/report/', options),
      read: (id, options = {}) => this._get(`/report/${id}/`, options)
    };
  }
  ...
  _initWebhooks() {
    this.webhook = {
      list: (options = {}) => this._get('/webhook/', options),
  ...
  _initEmailThreads() {
    this.email_thread = {
      list: (options = {}) => this._get('/email_thread/', options),
      read: (id) => this._get(`/email_thread/${id}/`)
    };
  }

  _initConnectedAccounts() {
    this.connected_account = {
      list: (options = {}) => this._get('/connected_account/', options),
  ...
  _initImport() {
    this.import = {
      create: (data) => this._post('/import/', data),
      read: (id) => this._get(`/import/${id}/`)
    };
  }
    ...
    try {
      ...
      if (response.status === 429) {
        if (retryCount < this.maxRetries) {
          const retryAfter = parseInt(response.headers.get('Retry-After') || this.retryDelay / 1000);
